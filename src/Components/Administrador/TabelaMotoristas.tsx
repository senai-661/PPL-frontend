                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            type="button"
                            onClick={async () => {
                              const confirmado = window.confirm(`Deseja realmente excluir o motorista ${m.idMotorista ?? '-'}?`);
                              if (!confirmado || m.idMotorista == null) return;

                              const sucesso = await MotoristaRequest.removerMotoristaPorAdmin(m.idMotorista);
                              if (sucesso) {
                                fetchMotoristas();
                              } else {
                                window.alert('Erro ao excluir o motorista.');
                              }
                            }}
